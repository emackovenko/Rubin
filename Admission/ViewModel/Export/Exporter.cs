                ...
                if (claim.OrphanDocuments.Count > 0)
                {
                    ...
                    foreach (var orphanDoc in claim.OrphanDocuments)
                    {
                        ...
                        var fisOrphanDoc = new Model.ExportData.OrphanDocument
                        {
                            ...
                            OriginalReceivedDate = ConvertDate(orphanDoc.OriginalReceivedDate),
                            OrphanCategoryID = orphanDoc.OrphanDocumentType.ExportCode,
                            DocumentName = orphanDoc.OrphanDocumentType.Name,
                            DocumentSeries = orphanDoc.Series,
                            DocumentNumber = orphanDoc.Number,
        ...
        public void FillOrderPackage(Root root)
		{
			...
			foreach (var order in collection)
			{
				var admissionOrder = new OrderOfAdmission
				{
					...
					EducationFormID = int.Parse(order.EducationForm.ExportCode),
					FinanceSourceID = int.Parse(order.FinanceSource.ExportCode),
					EducationLevelID = 2,
